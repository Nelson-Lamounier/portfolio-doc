...
type _PkgInterop = typeof pkg & { default?: { createAutocomplete: typeof pkg.createAutocomplete } }
const createAutocomplete = pkg?.createAutocomplete ?? (pkg as unknown as _PkgInterop)?.default?.createAutocomplete
  ...
  return (
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm text-zinc-700 group-aria-selected:text-teal-600 dark:text-zinc-300 dark:group-aria-selected:text-teal-400"
      >
      ...
      {hierarchy.length > 0 && (
        <div
          id={`${id}-hierarchy`}
          aria-hidden="true"
          className="mt-0.5 truncate text-xs whitespace-nowrap text-zinc-500 dark:text-zinc-400"
        >
  ...
  if (collection.items.length === 0) {
    return (
      <p className="px-4 py-8 text-center text-sm text-zinc-700 dark:text-zinc-400">
        ...
        <span className="wrap-break-word text-zinc-900 dark:text-white">
          {query}
        </span>
        &rdquo;
      </p>
...
>(function SearchInput({ autocomplete, autocompleteState, onClose }, inputRef) {
  ...
  return (
    <div className="group relative flex h-12">
      <SearchIcon className="pointer-events-none absolute top-0 left-4 h-full w-5 fill-zinc-400 dark:fill-zinc-500" />
      <input
        ref={inputRef}
        data-autofocus
        className={clsx(
          'flex-auto appearance-none bg-transparent pl-12 text-zinc-900 outline-hidden placeholder:text-zinc-400 focus:w-full focus:flex-none sm:text-sm dark:text-white [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-decoration]:hidden [&::-webkit-search-results-button]:hidden [&::-webkit-search-results-decoration]:hidden',
...
function CloseOnNavigation({
  ...
  useEffect(() => {
    close(autocomplete)
  }, [location.href, close, autocomplete])
    ...
    <>
      ...
      >
        ...
        <div className="fixed inset-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-20 md:py-32 lg:px-8 lg:py-[15vh]">
          <DialogPanel className="mx-auto transform-gpu overflow-hidden rounded-xl bg-white shadow-xl sm:max-w-xl dark:bg-zinc-900 dark:ring-1 dark:ring-zinc-700">
            <div {...autocomplete.getRootProps({})}>
              ...
              >
                ...
                <div
                  ref={panelRef}
                  className="border-t border-zinc-200 bg-white px-2 py-3 empty:hidden dark:border-zinc-400/10 dark:bg-zinc-900"
  ...
  return (
    <>
      <button
        ...
        {...buttonProps}
      >
        <SearchIcon className="h-5 w-5 flex-none fill-zinc-400 group-hover:fill-zinc-500 md:group-hover:fill-zinc-400 dark:fill-zinc-500" />
